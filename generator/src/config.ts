...
export interface GeneratorConfig {
  ...
  siteIds: number[];
  types: string[];
  duration: number; // in seconds, 0 = infinite
  dryRun: boolean;
  subscribe: boolean;
}

export function parseConfig(): GeneratorConfig {
  ...
  let siteIds: number[] = [];
  const rawSiteIds: string = opts.siteIds;
  ...
  const types = (opts.types as string).split(',').map((t) => t.trim().toUpperCase()).filter(Boolean);
